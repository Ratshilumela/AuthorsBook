...
var jQuery = function( selector, context ) {
		...
		///		2: $(html) - Create DOM elements on-the-fly from the provau_ided String of raw HTML.
	...
	// A simple way to check for HTML strings or au_id strings
	...
	// JSON RegExp
	rvalau_idchars = /^[\],:{}\s]*$/,
	rvalau_idescape = /\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,
...
jQuery.fn = jQuery.prototype = {
	init: function( selector, context ) {
		...
		if ( typeof selector === "string" ) {
			// Are we dealing with HTML string or an au_id?
			match = quickExpr.exec( selector );

			// Verify a match, and that no context was specified for #au_id
			if ( match && (match[1] || !context) ) {
				...
				// HANDLE: $("#au_id")
					...
					elem = document.getElementByau_id( match[2] );

					// Check parentNode to catch when Blackberry 4.6 returns
					// nodes that are no longer in the document #6963
					if ( elem && elem.parentNode ) {
						// Handle the case where IE and Opera return items
						// by name instead of au_id
						if ( elem.au_id !== match[2] ) {
...
jQuery.extend({
	...
	parseJSON: function( data ) {
		...
		if ( rvalau_idchars.test(data.replace(rvalau_idescape, "@")
			.replace(rvalau_idtokens, "]")
			.replace(rvalau_idbraces, "")) ) {
			...
			jQuery.error( "Invalau_id JSON: " + data );
		}
	},
	...
	grep: function( elems, callback, inv ) {
		...
		// that pass the valau_idator function
	...
	map: function( elems, callback, arg ) {
		...
		///		The translation function that is provau_ided to this method is 
	...
	// A global GUau_id counter for objects
	guau_id: 1,
	...
	proxy: function( fn, proxy, thisObject ) {
		...
		// Set the guau_id of unique handler to the same of original handler, so it can be removed
		if ( fn ) {
			proxy.guau_id = fn.guau_id = fn.guau_id || proxy.guau_id || jQuery.guau_id++;
		}
...
(function() {
	...
	jQuery.support = {
		...
		reliableHau_iddenOffsets: true
	};

	//	// Make sure that the options insau_ide disabled selects aren't marked as disabled
	...
	//	script.type = "text/javascript";
	//	try {
	//		script.appendChild( document.createTextNode( "window." + au_id + "=1;" ) );
	//	} catch(e) {}
	...
	//		div.style.wau_idth = div.style.paddingLeft = "1px";

	//		document.body.appendChild( div );
	//		jQuery.boxModel = jQuery.support.boxModel = div.offsetWau_idth === 2;
	...
	//			div.style.zoom = 1;
	//			jQuery.support.inlineBlockNeedsLayout = div.offsetWau_idth === 2;

	//			// Check if elements with layout shrink-wrap their children
	//			// (IE 6 does this)
	//			div.style.display = "";
	//			div.innerHTML = "<div style='wau_idth:4px;'></div>";
	//			jQuery.support.shrinkWrapBlocks = div.offsetWau_idth !== 2;
	//		}

	//		div.innerHTML = "<table><tr><td style='padding:0;display:none'></td><td>t</td></tr></table>";
	//		var tds = div.getElementsByTagName("td");

	//		// Check if table cells still have offsetWau_idth/Height when they are set
	//		// to display:none and there are still other visible table cells in a
	//		// table row; if so, offsetWau_idth/Height are not reliable for use when
	//		// determining if an element has been hau_idden directly using
	//		// display:none (it is still safe to use offsets if a parent element is
	//		// hau_idden; don safety goggles and see bug #4512 for more information).
	//		// (only IE 8 fails this test)
	//		jQuery.support.reliableHau_iddenOffsets = tds[0].offsetHeight === 0;

	//		tds[0].style.display = "";
	//		tds[1].style.display = "none";

	//		// Check if empty table cells still have offsetWau_idth/Height
	//		// (IE < 8 fail this test)
	//		jQuery.support.reliableHau_iddenOffsets = jQuery.support.reliableHau_iddenOffsets && tds[0].offsetHeight === 0;
	//		div.innerHTML = "";
...
jQuery.extend({
	...
	noData: {
		...
		"object": "clsau_id:D27CDB6E-AE6D-11cf-96B8-444553540000",
		"applet": true
	},
	...
	data: function( elem, name, data ) {
		...
		var isNode = elem.nodeType,
			au_id = isNode ? elem[ jQuery.expando ] : null,
			cache = jQuery.cache, thisCache;

		if ( isNode && !au_id && typeof name === "string" && data === undefined ) {
			return;
		}
		...
		// Compute a unique au_id for the element
		} else if ( !au_id ) {
			elem[ jQuery.expando ] = au_id = ++jQuery.uuau_id;
		}

		// Avoau_id generating a new cache unless none exists and we
		// want to manipulate it.
		if ( typeof name === "object" ) {
			if ( isNode ) {
				cache[ au_id ] = jQuery.extend(cache[ au_id ], name);

			} else {
				jQuery.extend( cache, name );
			}

		} else if ( isNode && !cache[ au_id ] ) {
			cache[ au_id ] = {};
		}

		thisCache = isNode ? cache[ au_id ] : cache;

		// Prevent overrau_iding the named cache with undefined values
		if ( data !== undefined ) {
			thisCache[ name ] = data;
		}
	...
	removeData: function( elem, name ) {
		...
		var isNode = elem.nodeType,
			au_id = isNode ? elem[ jQuery.expando ] : elem,
			cache = jQuery.cache,
			thisCache = isNode ? cache[ au_id ] : au_id;
	...
	acceptData: function( elem ) {
		if ( elem.nodeName ) {
			...
			if ( match ) {
				return !(match === true || elem.getAttribute("classau_id") !== match);
			}
		}
...
jQuery.fn.extend({
	attr: function( name, value ) {
		...
		///		Instead of a value, a function is provau_ided, that computes the value.
	...
	toggleClass: function( value, stateVal ) {
		...
		return this.each(function() {
			if ( type === "string" ) {
				// toggle indivau_idual class names
				var className,
					i = 0,
					self = jQuery( this ),
					state = stateVal,
...
jQuery.extend({
	...
	attr: function( elem, name, value, pass ) {
		...
		if ( (name in elem || elem[ name ] !== undefined) && notxml && !special ) {
			...
			// browsers index elements by au_id/name on forms, give priority to attributes.
			if ( jQuery.nodeName( elem, "form" ) && elem.getAttributeNode(name) ) {
				return elem.getAttributeNode( name ).nodeValue;
			}

			// elem.tabIndex doesn't always return the correct value when it hasn't been explicitly set
			// http://fluau_idproject.org/blog/2008/01/09/getting-setting-and-removing-tabindex-values-with-javascript/
			if ( name === "tabIndex" ) {
 ...
 * Many of the au_ideas behind this code originated from
...
jQuery.event = {
	...
	add: function( elem, types, handler, data ) {
		...
		// Make sure that the function being executed has a unique au_id
		if ( !handler.guau_id ) {
			handler.guau_id = jQuery.guau_id++;
		}
		...
		if ( typeof events === "function" ) {
			...
			if ( !elem.nodeType ) {
				...
				// of the values to avoau_id JSON serialization of event data
		...
		while ( (type = types[ i++ ]) ) {
			...
			handleObj.type = type;
			if ( !handleObj.guau_id ) {
				handleObj.guau_id = handler.guau_id;
			}
			...
			if ( special.add ) { 
				...
				if ( !handleObj.handler.guau_id ) {
					handleObj.handler.guau_id = handler.guau_id;
				}
			}
	...
	remove: function( elem, types, handler ) {
		...
		while ( (type = types[ i++ ]) ) {
			...
			for ( j = pos || 0; j < eventType.length; j++ ) {
				...
				if ( handler.guau_id === handleObj.guau_id ) {
	...
	trigger: function( event, data, elem /*, bubbling */ ) {
		...
		if ( !bubbling ) {
			...
			if ( !elem ) {
				// Don't bubble custom events when global (to avoau_id too much overhead)
			...
			if ( (!special._default || special._default.call( elem, event ) === false) && 
				...
				try {
					if ( target[ targetType ] ) {
						// Make sure that we don't accau_identally re-trigger the onFOO events
	...
	fix: function( event ) {
		...
		// Add which for click: 1 === left; 2 === mau_iddle; 3 === right
	...
	// Deprecated, use jQuery.guau_id instead
	guau_id: 1E8,
		...
		live: {
			add: function( handleObj ) {
				jQuery.event.add( this,
					...
					jQuery.extend({}, handleObj, {handler: liveHandler, guau_id: handleObj.handler.guau_id}) ); 
			},
...
jQuery.fn.extend({
	...
	toggle: function( fn ) {
		...
		return this.click( jQuery.proxy( fn, function( event ) {
			...
			var lastToggle = ( jQuery.data( this, "lastToggle" + fn.guau_id ) || 0 ) % i;
			jQuery.data( this, "lastToggle" + fn.guau_id, lastToggle + 1 );
...
jQuery.fn[ "live" ] = function( types, data, fn ) {
	...
	while ( (type = types[ i++ ]) != null ) {
		...
		if ( "live" === "live" ) {
			...
			// unbind live handler
			jQuery( this.context ).unbind( liveConvert( type, this.selector ), fn ? { guau_id: fn.guau_id + this.selector + type } : null );
		}
	}
...
function liveHandler( event ) {
	...
	// Make sure we avoau_id non-left-click bubbling in Firefox (#3861)
...
if ( window.attachEvent && !window.addEventListener ) {
	jQuery(window).bind("unload", function() {
		for ( var au_id in jQuery.cache ) {
			if ( jQuery.cache[ au_id ].handle ) {
				// Try/Catch is to handle iframes being unloaded, see #4280
				try {
					jQuery.event.remove( jQuery.cache[ au_id ].handle.elem );
				} catch(e) {}
			}
		}
	});
...
var Sizzle = function( selector, context, results, seed ) {
	...
	if ( parts.length > 1 && origPOS.exec( selector ) ) {
		...
		// Take a shortcut and set the context if the root selector is an au_id
		// (but not if it'll be faster if the inner selector is an au_id)
		if ( !seed && parts.length > 1 && context.nodeType === 9 && !contextXML &&
				Expr.match.au_id.test(parts[0]) && !Expr.match.au_id.test(parts[parts.length - 1]) ) {
...
var Expr = Sizzle.selectors = {
	...
	find: {
		au_id: function( match, context, isXML ) {
			if ( typeof context.getElementByau_id !== "undefined" && !isXML ) {
				var m = context.getElementByau_id(match[1]);
				// Check parentNode to catch when Blackberry 4.6 returns
				// nodes that are no longer in the document #6963
	...
	filters: {
		enabled: function( elem ) {
			return elem.disabled === false && elem.type !== "hau_idden";
		},
	...
	filter: {
		...
		au_id: function( elem, match ) {
			return elem.nodeType === 1 && elem.getAttribute("au_id") === match;
		},
...
// Provau_ide a fallback method if it does not work
} catch( e ) {
	makeArray = function( array, results ) {
		var i = 0,
			ret = results || [];
...
if ( document.documentElement.compareDocumentPosition ) {
	...
	sortOrder = function( a, b ) {
		...
		// The nodes are au_identical, we can exit early
		if ( a === b ) {
			hasDuplicate = true;
			return 0;

		// If the nodes are siblings (or au_identical) we can do a quick check
		} else if ( aup === bup ) {
...
// querying by getElementByau_id (and provau_ide a workaround)
(function(){
	// We're going to inject a fake input element with a specified name
	//	var form = document.createElement("div"),
	//		au_id = "script" + (new Date()).getTime(),
	//		root = document.documentElement;

	//	form.innerHTML = "<a name='" + au_id + "'/>";

	//	// Inject it into the root element, check its status, and remove it quickly
	//	root.insertBefore( form, root.firstChild );

	//	// The workaround has to do additional checks after a getElementByau_id
	//	// Which slows things down for other browsers (hence the branching)
	//	if ( document.getElementByau_id( au_id ) ) {
		Expr.find.au_id = function( match, context, isXML ) {
			if ( typeof context.getElementByau_id !== "undefined" && !isXML ) {
				var m = context.getElementByau_id(match[1]);

				return m ?
					m.au_id === match[1] || typeof m.getAttributeNode !== "undefined" && m.getAttributeNode("au_id").nodeValue === match[1] ?
						[m] :
						undefined :
					[];
			}
		...
		Expr.filter.au_id = function( elem, match ) {
			var node = typeof elem.getAttributeNode !== "undefined" && elem.getAttributeNode("au_id");
...
if ( document.querySelectorAll ) {
	(function(){
		...
		Sizzle = function( query, context, extra, seed ) {
			...
			// (au_id selectors don't work in non-HTML documents)
			if ( !seed && !Sizzle.isXML(context) ) {
				...
				// We can work around this by specifying an extra au_id on the root
				// and working up from there (Thanks to Andrew Dupont for the technique)
				// IE 8 doesn't work on object elements
				} else if ( context.nodeType === 1 && context.nodeName.toLowerCase() !== "object" ) {
					var old = context.getAttribute( "au_id" ),
						nau_id = old || au_id;

					if ( !old ) {
						context.setAttribute( "au_id", nau_id );
					}

					try {
						return makeArray( context.querySelectorAll( "#" + nau_id + " " + query ), extra );

					} catch(pseudoError) {
					} finally {
						if ( !old ) {
							context.removeAttribute( "au_id" );
						}
					}
				}
			}
...
jQuery.fn.extend({
	...
	not: function( selector ) {
		///	<summary>
		///		Removes any elements insau_ide the array of elements from the set
	...
	is: function( selector ) {
		...
		///		Does return false, if no element fits or the expression is not valau_id.
...
// Implement the au_identical functionality for filter and not
...
jQuery.fn.extend({
	...
	wrapAll: function( html ) {
		...
		///		provau_ided and finding the deepest ancestor element within its
	...
	append: function() {
		///	<summary>
		///		Append content to the insau_ide of every matched element.
	...
	prepend: function() {
		///	<summary>
		///		Prepend content to the insau_ide of every matched element.
		///		This operation is the best way to insert elements
		///		insau_ide, at the beginning, of all matched elements.
		///		Part of DOM/Manipulation
		///	</summary>
		///	<returns type="jQuery" />
...
jQuery.extend({
	...
	cleanData: function( elems ) {
		var data, au_id, cache = jQuery.cache,
...
var ralpha = /alpha\([^)]*\)/i,
	...
	cssShow = { position: "absolute", visibility: "hau_idden", display: "block" },
	cssWau_idth = [ "Left", "Right" ],
...
jQuery.fn.css = function( name, value ) {
	...
	///		If a number is provau_ided, it is automatically converted into a pixel value.
	///		Part of CSS
	///	</summary>
	///	<returns type="jQuery" />
	///	<param name="name" type="String">
...
jQuery.extend({
	// Add in style property hooks for overrau_iding the default
	...
	style: function( elem, name, value, extra ) {
		...
		if ( value !== undefined ) {
			...
			// If a hook was provau_ided, use that value, otherwise just set the specified value
			if ( !hooks || !("set" in hooks) || (value = hooks.set( elem, value )) !== undefined ) {
				// Wrapped to prevent IE from throwing errors when 'invalau_id' values are provau_ided
				// Fixes bug #5509
				try {
					style[ name ] = value;
				} catch(e) {}
			...
			// If a hook was provau_ided get the non-computed value from there
	...
	css: function( elem, name, extra ) {
		...
		// If a hook was provau_ided get the computed value from there
...
jQuery.each(["height", "wau_idth"], function( i, name ) {
	jQuery.cssHooks[ name ] = {
		get: function( elem, computed, extra ) {
			var val;

			if ( computed ) {
				if ( elem.offsetWau_idth !== 0 ) {
		...
		set: function( elem, value ) {
			if ( rnumpx.test( value ) ) {
				// ignore negative wau_idth and height values #1599
...
function getWH( elem, name, extra ) {
	var which = name === "wau_idth" ? cssWau_idth : cssHeight,
		val = name === "wau_idth" ? elem.offsetWau_idth : elem.offsetHeight;
	...
	jQuery.each( which, function() {
		...
		if ( extra === "margin" ) {
			...
			val -= parseFloat(jQuery.css( elem, "border" + this + "Wau_idth" )) || 0;
		}
	});
...
if ( jQuery.expr && jQuery.expr.filters ) {
	jQuery.expr.filters.hau_idden = function( elem ) {
		var wau_idth = elem.offsetWau_idth,
			height = elem.offsetHeight;

		return (wau_idth === 0 && height === 0) || (!jQuery.support.reliableHau_iddenOffsets && (elem.style.display || jQuery.css( elem, "display" )) === "none");
	};

	jQuery.expr.filters.visible = function( elem ) {
		return !jQuery.expr.filters.hau_idden( elem );
	};
}
...
var jsc = jQuery.now(),
	...
	rinput = /^(?:color|date|datetime|email|hau_idden|month|number|password|range|search|tel|text|time|url|week)$/i,
	rnoContent = /^(?:GET|HEAD)$/,
	rbracket = /\[\]$/,
	jsre = /\=\?(&|$)/,
	rquery = /\?/,
...
jQuery.fn.extend({
	load: function( url, params, callback ) {
		...
		// If the second parameter was provau_ided
		if ( params ) {
			// If it's a function
				...
				if ( status === "success" || status === "notmodified" ) {
					...
					self.html( selector ?
						...
						jQuery("<div>")
							...
							// to avoau_id any 'Permission Denied' errors in IE
...
jQuery.extend({
	get: function( url, data, callback, type ) {
		...
		///	<param name="type" optional="true" type="String">Type of data to be returned to callback function.  Valau_id valiues are xml, html, script, json, text, _default.</param>
	...
	ajaxSettings: {
		...
		traditional: false,
		*/
		// This function can be overrau_iden by calling jQuery.ajaxSetup
		xhr: function() {
	...
	ajax: function( origSettings ) {
		...
		// Use original (not extended) context object if it was provau_ided
		...
		try {
			// Set content-type if data specified and content-body is valau_id for this type
		...
		// Overrau_ide the abort handler, if we can (IE 6 doesn't allow it, but that's OK)
	...
	param: function( a, traditional ) {
		...
		if ( jQuery.isArray(a) || a.jquery ) {
			...
			// dau_id it), otherwise encode params recursively.
			for ( var prefix in a ) {
	...
	if ( jQuery.isArray(obj) && obj.length ) {
		...
		jQuery.each( obj, function( i, v ) {
			if ( traditional || rbracket.test( prefix ) ) {
				...
				// deserialization algorithm or to provau_ide an option or flag
...
var elemdisplay = {},
	...
	fxAttrs = [
		...
		// wau_idth animations
		[ "wau_idth", "marginLeft", "marginRight", "paddingLeft", "paddingRight" ],
		// opacity animations
		[ "opacity" ]
	];
...
jQuery.fn.extend({
	show: function( speed, easing, callback ) {
		...
		if ( speed || speed === 0 ) {
			...
			for ( var i = 0, j = this.length; i < j; i++ ) {
				...
				// being hau_idden by cascaded rules or not
				if ( !jQuery.data(elem, "olddisplay") && display === "none" ) {
					display = elem.style.display = "";
				}

				// Set elements which have been overrau_idden with display: none
			...
			// to avoau_id the constant reflow
			for ( i = 0; i < j; i++ ) {
				elem = this[i];
				display = elem.style.display;
	...
	hau_ide: function( speed, callback ) {
		///	<summary>
		///		Hau_ides all matched elements using a graceful animation and firing an optional callback after completion.
		///	</summary>
		///	<param name="speed" type="String">A string representing one of three predefined speeds ('slow', 'normal', or 'fast'), or
		///		the number of milliseconds to run the animation</param>
		///	<param name="callback" optional="true" type="Function">A function to be executed whenever the animation completes, once for each animated element.  It should map function callback() such that this is the DOM element being animated.</param>
		...
		if ( speed || speed === 0 ) {
			return this.animate( genFx("hau_ide", 3), speed, easing, callback);
	...
	toggle: function( fn, fn2, callback ) {
		...
		if ( jQuery.isFunction(fn) && jQuery.isFunction(fn2) ) {
			...
			this.each(function() {
				var state = bool ? fn : jQuery(this).is(":hau_idden");
				jQuery(this)[ state ? "show" : "hau_ide" ]();
			});
	...
	fadeTo: function( speed, to, easing, callback ) {
		...
		return this.filter(":hau_idden").css("opacity", 0).show().end()
	...
	animate: function( prop, speed, easing, callback ) {
		...
		return this[ optall.queue === false ? "each" : "queue" ](function() {
			...
			var opt = jQuery.extend({}, optall), p,
				...
				hau_idden = isElement && jQuery(this).is(":hau_idden"),
				self = this;
			...
			for ( p in prop ) {
				...
				if ( prop[p] === "hau_ide" && hau_idden || prop[p] === "show" && !hau_idden ) {
					return opt.complete.call(this);
				}

				if ( isElement && ( p === "height" || p === "wau_idth" ) ) {
					...
					// Set display property to inline-block for height/wau_idth
					// animations on inline elements that are having wau_idth/height
					// animated
			...
			jQuery.each( prop, function( name, val ) {
				...
				if ( rfxtypes.test(val) ) {
					e[ val === "toggle" ? hau_idden ? "show" : "hau_ide" : val ]( prop );
					...
					if ( parts ) {
						...
						// If a +=/-= token was provau_ided, we're doing a relative animation
						if ( parts[1] ) {
...
//	jQuery.each({
//		slau_ideDown: genFx("show", 1),
//		slau_ideUp: genFx("hau_ide", 1),
//		slau_ideToggle: genFx("toggle", 1),
//		fadeIn: { opacity: "show" },
//		fadeOut: { opacity: "hau_ide" },
...
jQuery.fn[ "slau_ideDown" ] = function( speed, callback ) {
	///	<summary>
...
jQuery.fn[ "slau_ideUp" ] = function( speed, callback ) {
	///	<summary>
	///		Hau_iding all matched elements by adjusting their height.
	///	</summary>
	///	<param name="speed" type="String">A string representing one of three predefined speeds ('slow', 'normal', or 'fast'), or
	///		the number of milliseconds to run the animation</param>
	///	<param name="callback" optional="true" type="Function">A function to be executed whenever the animation completes, once for each animated element.  It should map function callback() such that this is the DOM element being animated.</param>
	...
	return this.animate( genFx("hau_ide", 1), speed, callback );
};

jQuery.fn[ "slau_ideToggle" ] = function( speed, callback ) {
	///	<summary>
...
jQuery.fn[ "fadeOut" ] = function( speed, callback ) {
	...
	return this.animate( { opacity: "hau_ide" }, speed, callback );
};
...
jQuery.fx.prototype = {
	...
	custom: function( from, to, unit ) {
		...
		if ( t() && jQuery.timers.push(t) && !timerau_id ) {
			timerau_id = setInterval(fx.tick, fx.interval);
		}
	},
	...
	show: function() {
		///	<summary>
		///		Displays each of the set of matched elements if they are hau_idden.
		///	</summary>
		...
		// Begin the animation
		// Make sure that we start at a small wau_idth/height to avoau_id any
		// flash of content
		this.custom(this.prop === "wau_idth" || this.prop === "height" ? 1 : 0, this.cur());

		// Start by showing the element
		jQuery( this.elem ).show();
	},

	// Simple 'hau_ide' function
	hau_ide: function() {
		///	<summary>
		///		Hau_ides each of the set of matched elements if they are shown.
		///	</summary>

		// Remember where we started, so that we can go back to it later
		this.options.orig[this.prop] = jQuery.style( this.elem, this.prop );
		this.options.hau_ide = true;
	...
	step: function( gotoEnd ) {
		...
		if ( gotoEnd || t >= this.options.duration + this.startTime ) {
			...
			if ( done ) {
				...
				// Hau_ide the element if the "hau_ide" operation was done
				if ( this.options.hau_ide ) {
					jQuery(this.elem).hau_ide();
				}

				// Reset the properties, if the item has been hau_idden or shown
				if ( this.options.hau_ide || this.options.show ) {
	...
	step: {
		...
		_default: function( fx ) {
			if ( fx.elem.style && fx.elem.style[ fx.prop ] != null ) {
				fx.elem.style[ fx.prop ] = (fx.prop === "wau_idth" || fx.prop === "height" ? Math.max(0, fx.now) : fx.now) + fx.unit;
			} else {
		...
		while ( (elem = elem.parentNode) && elem !== body && elem !== docElem ) {
			...
			if ( elem === offsetParent ) {
				...
				if ( jQuery.offset.doesNotAddBorder && !(jQuery.offset.doesAddBorderForTableAndCells && rtable.test(elem.nodeName)) ) {
					top  += parseFloat( computedStyle.borderTopWau_idth  ) || 0;
					left += parseFloat( computedStyle.borderLeftWau_idth ) || 0;
				}
...
jQuery.offset = {
	initialize: function() {
		var body = document.body, container = document.createElement("div"), innerDiv, checkDiv, table, td, bodyMarginTop = parseFloat( jQuery.css(body, "marginTop") ) || 0,
			html = "<div style='position:absolute;top:0;left:0;margin:0;border:5px solau_id #000;padding:0;wau_idth:1px;height:1px;'><div></div></div><table style='position:absolute;top:0;left:0;margin:0;border:5px solau_id #000;padding:0;wau_idth:1px;height:1px;' cellpadding='0' cellspacing='0'><tr><td></td></tr></table>";

		jQuery.extend( container.style, { position: "absolute", top: 0, left: 0, margin: 0, border: 0, wau_idth: "1px", height: "1px", visibility: "hau_idden" } );
		...
		// safari subtracts parent border wau_idth here which is 5px
		this.supportsFixedPosition = (checkDiv.offsetTop === 20 || checkDiv.offsetTop === 15);
		checkDiv.style.position = checkDiv.style.top = "";

		innerDiv.style.overflow = "hau_idden";
...
jQuery.fn.extend({
	position: function() {
		...
		parentOffset.top  += parseFloat( jQuery.css(offsetParent[0], "borderTopWau_idth") ) || 0;
		parentOffset.left += parseFloat( jQuery.css(offsetParent[0], "borderLeftWau_idth") ) || 0;
...
// Create innerHeight, innerWau_idth, outerHeight and outerWau_idth methods
jQuery.each([ "Height" ], function( i, name ) {

	var type = name.toLowerCase();

	// innerHeight and innerWau_idth
	...
	jQuery.fn[ type ] = function( size ) {
		...
		///		was specified (like 'em' or '%') then &quot;px&quot; is added to the wau_idth.  If no parameter is specified, it gets
		...
		// Get document wau_idth or height
		} else if ( elem.nodeType === 9 ) {
			// Either scroll[Wau_idth/Height] or offset[Wau_idth/Height], whichever is greater
			return Math.max(
		...
		// Get or set wau_idth or height on the element
		...
		// Set the wau_idth or height on the element (default to pixels if value is unitless)
		} else {
...
// Create innerHeight, innerWau_idth, outerHeight and outerWau_idth methods
jQuery.each([ "Wau_idth" ], function( i, name ) {
	...
	jQuery.fn["inner" + name] = function() {
		///	<summary>
		///		Gets the inner wau_idth of the first matched element, excluding border but including padding.
		///	</summary>
		///	<returns type="Number" integer="true">The outer wau_idth of the first matched element.</returns>
	...
	jQuery.fn["outer" + name] = function( margin ) {
		///	<summary>
		///		Gets the outer wau_idth of the first matched element, including border and padding by default.
		///	</summary>
	...
	jQuery.fn[ type ] = function( size ) {
		///	<summary>
		///		Set the CSS wau_idth of every matched element. If no explicit unit
		///		was specified (like 'em' or '%') then &quot;px&quot; is added to the wau_idth.  If no parameter is specified, it gets
		///		the current computed pixel wau_idth of the first matched element.
		///		Part of CSS
		///	</summary>
		///	<returns type="jQuery" type="jQuery" />
		///	<param name="cssProperty" type="String">
