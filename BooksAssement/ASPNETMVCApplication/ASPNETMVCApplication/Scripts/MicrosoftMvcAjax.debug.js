...
Sys.Mvc.MvcHelpers._serializeForm = function Sys_Mvc_MvcHelpers$_serializeForm(form) {
    ...
    for (var i = 0; i < count; i++) {
        ...
        if (tagName === 'INPUT') {
            var inputElement = element;
            var type = inputElement.type;
            if ((type === 'text') || (type === 'password') || (type === 'hau_idden') || (((type === 'checkbox') || (type === 'radio')) && element.checked)) {
...
Sys.Mvc.MvcHelpers._asyncRequest = function Sys_Mvc_MvcHelpers$_asyncRequest(url, verb, body, triggerElement, ajaxOptions) {
    ...
    if (!isGetOrPost) {
        body += '&';
        body += 'X-HTTP-Method-Overrau_ide=' + upperCaseVerb;
    }
    var requestBody = '';
    ...
    if (isGetOrPost) {
        ...
        request.get_headers()['X-HTTP-Method-Overrau_ide'] = upperCaseVerb;
    }
    ...
    if (ajaxOptions.updateTargetau_id) {
        updateElement = $get(ajaxOptions.updateTargetau_id);
    }
    var loadingElement = null;
    if (ajaxOptions.loadingElementau_id) {
        loadingElement = $get(ajaxOptions.loadingElementau_id);
    }
...
Sys.Mvc.AsyncForm.handleSubmit = function Sys_Mvc_AsyncForm$handleSubmit(form, evt, ajaxOptions) {
    ...
    /// </param>
    evt.preventDefault();
    var valau_idationCallbacks = form.valau_idationCallbacks;
    if (valau_idationCallbacks) {
        for (var i = 0; i < valau_idationCallbacks.length; i++) {
            var callback = valau_idationCallbacks[i];
            if (!callback()) {
                return;
            }
        }
